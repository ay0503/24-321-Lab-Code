...
def print_values() -> None:
    ...
    with open(f"Values Table.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerows(result)
...
def exercise_a_calculations():
    ...
    R_c_hot = (T_3 - T_4) / q_int - R_brass_34
    VALUES["R_c_hot"] = R_c_hot

    R_brass_56 = L_56 / (k_brass * A)
    R_c_cold = (T_5 - T_6) / q_int - R_brass_56
...
def figure_1_plot():
    ...
    # Line plots
    times = list(map(sample_to_time, xls["Sample Number"]))[:200]
    temps = list(xls["Temp T8 [deg C]"])[:200]
    temps_err = list(map(temp_err, temps))[:200]
...
###############################################################################
# EXERCISE A : q_elec Calculations
###############################################################################
